...
def pipe(value, *functions):
    """
    Applies a series of functions to an initial value, passing the result of
    each function as the argument to the next function in the sequence.
    ...
    Returns:
        The final result after all functions have been applied to the initial
        value.
...
def map_if(
    ...
    Args:
        condition (Callable[[_T], bool]): A function that takes an element of the
            iterable and returns True if the element should be mapped.

        func (Callable[[_T], _T]): A function that takes an element of the iterable
            and returns the mapped value.

        iterable (Iterable[_T]): An iterable of elements to be checked and
        potentially mapped.

    Returns:
        Iterable[_T]: A generator that yields elements from the original iterable,
        with mappings where the condition is satisfied.
    """
...
def first(condition: Callable[[_T], bool], iterable: Iterable[_T]):
    ...
    Args:
        `condition (Callable[[_T], bool])`: A function that takes an element of the
            iterable and returns True if the element satisfies the condition,
            otherwise False.
...
def first_or_default(
    ...
    Args:
        `condition (Callable[[_T], bool])`: A function that takes an element of
            the iterable and returns True if the element satisfies the condition,
            otherwise False.

        `iterable (Iterable[_T])`: An iterable of elements to be filtered.

        `default (_T)`: The default value to return if no element satisfies
            the condition.

    Returns:
        `_T`: The first element that satisfies the condition, or the default value
...
def replace(
    ...
    Args:
        `condition (Callable[[_T], bool])`: A function that takes an element of the
            iterable and returns True if the element should be replaced.

        `new_value (_T)`: The value to replace elements with if they satisfy the
            condition.

        `iterable (Iterable[_T])`: The iterable whose elements are to be checked and
            potentially replaced.

    Returns:
        `Iterable[_T]`: A generator that yields elements from the original iterable,
            with replacements where the condition is satisfied.
    """
...
def validate_dimension(
    ...
    Args:
        `value (str | pint.Quantity)`: The value to be validated. It can be
            a string or a pint.Quantity.

        `dimension (str)`: The expected dimension of the value.

        `conversion (Callable, optional)`: A function to convert the value
            to a `pint.Quantity`. Defaults to a lambda function that uses
